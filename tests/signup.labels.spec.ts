...
test.describe('Signup – Field Labels & UI Copy Tests', () => {
  ...
  test('TC-L-17 province select shows only the placeholder on page load [BUG-04]', async ({
    ...
    // Reduce race conditions: wait for init signals that may auto-set province,
    // but do not fail this wait if they are delayed/missed in a specific run.
    const geolocationSettled = page
      .waitForResponse(
    ...
    const initialValue = await signupPage.provinceSelect.inputValue();
    const provinceChanged = page
      .waitForFunction(
        ({ selector, initial }) => {
          const select = document.querySelector(selector) as HTMLSelectElement | null;
          return !!select && select.value !== initial;
        },
        { selector: '[data-testid="region-select"]', initial: initialValue },
        { timeout: 4_000 },
      )
      .catch(() => null);

    await Promise.allSettled([geolocationSettled, provinceChanged]);
